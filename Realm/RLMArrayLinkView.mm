...
@public
    ...
    RLMRealm *_realm;
    RLMObjectInfo *_containedObjectInfo;
...
+ (RLMArrayLinkView *)arrayWithObjectClassName:(NSString *)objectClassName
    ...
    ar->_realm = realm;
    ar->_containedObjectInfo = &ar->_realm->_info.find(objectClassName)->second; // FIXME: needs getter to throw if missing
...
- (NSUInteger)countByEnumeratingWithState:(NSFastEnumerationState *)state
    ...
    if (state->state == 0) {
        ...
        enumerator = [[RLMFastEnumerator alloc] initWithCollection:self objectSchema:_containedObjectInfo];
...
- (id)objectAtIndex:(NSUInteger)index {
    return RLMCreateObjectAccessor(_realm, *_containedObjectInfo,
...
- (id)valueForKeyPath:(NSString *)keyPath {
    if ([keyPath hasPrefix:@"@"]) {
        ...
        RLMResults *results = [RLMResults resultsWithObjectInfo:_containedObjectInfo
...
- (RLMResults *)sortedResultsUsingDescriptors:(NSArray *)properties {
    auto order = RLMSortOrderFromDescriptors(_containedObjectInfo->rlmObjectSchema, properties);
    auto results = translateErrors([&] { return _backingList.sort(std::move(order)); });
    return [RLMResults resultsWithObjectInfo:_containedObjectInfo results:std::move(results)];
}

- (RLMResults *)objectsWithPredicate:(NSPredicate *)predicate {
    auto query = translateErrors([&] { return _backingList.get_query(); });
    RLMUpdateQueryWithPredicate(&query, predicate, _realm.schema, _containedObjectInfo->rlmObjectSchema);
    return [RLMResults resultsWithObjectInfo:_containedObjectInfo
